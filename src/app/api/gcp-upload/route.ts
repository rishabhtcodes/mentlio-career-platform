...
export async function GET() {
  try {
    if (!process.env.GCP_SERVICE_ACCOUNT_KEY || !process.env.GCP_BUCKET_ASSETS) {
      return NextResponse.json({ error: "GCP credentials or bucket not configured" }, { status: 500 });
    }
